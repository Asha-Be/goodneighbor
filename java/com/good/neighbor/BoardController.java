...
import org.springframework.ui.Model;//값을 담아서 front-end로 보낸다 
//									return 뷰이름;
...
public class BoardController {
		...
		public String writeForm(Model model, HttpServletRequest request) {
			...
			//return "/board/writeForm";//뷰리턴 views/board/writeForm.jsp
		...
		public String writePro(@ModelAttribute("boardDTO") BoardDTO boardDTO,
			...
			if(maxNum != 0) {
				maxNum=maxNum+1;//답글 그룹으로 사용하려고 ref에 넣을려고
			}else {//처음 글이면 
				maxNum=1;
			}
			...
			boardDTO.setBoard_ip(ip);//setter 작업 
			
			if(boardDTO.getBoard_num() != 0) {//답글이면 
				...
				boardDTO.setBoard_ref(new Integer(maxNum));//글그룹
			...
			return "redirect:/board/list.do";//url 주소
		...
		@RequestMapping("list.do")
		   public String listBoard(@ModelAttribute("boardDTO") BoardDTO boardDTO, Model model, HttpServletRequest request, 
		     ...
		     if(request.getParameter("keyWord")!=null) {
		      ...
		      //int cnt=sqlSession.selectOne("noticeDAO.countNotice");//총 글 갯수
		        int cnt=0;
		...
		public String contentDo(Model model,HttpServletRequest request) {
			...
			//return "/board/content"; //뷰리턴, content.jsp
		...
		public ModelAndView updateForm(HttpServletRequest request) {
			...
			//mv.setViewName("/board/updateForm");//뷰리턴 updateFor.jsp
		...
		public ModelAndView updatePro(@ModelAttribute("boardDTO") BoardDTO boardDTO,
			...
			String pageNum=request.getParameter("pageNum");
			sqlSession.update("board.updateBoard",boardDTO);//글수정
