...
<body>
	...
	<form method="post" name="writeForm" action="${ctxpath}/board/writePro.do" onSubmit="return writeSave()">
	  ...
	  <table border="1">
	  	<tr>
	  		<td>글쓴이</td>
	  		<td><input type="text" name="board_writer" id="board_writer" size="30"></td>
		</tr>
		...
		<tr>
	  		...
	  		<!-- 원글 -->
	  			<c:if test="${board_num==0}">
	  			<input type="text" name="board_subject" id="board_subject" size="40">
	  			</c:if>
	  			<!--  답글 -->
	  			<c:if test="${board_num!=0 }">
	  			<input type="text" name="board_subject" id="board_subject" size="40" value="[답변]">
	  			</c:if>
		...
		<tr>
	  		<td>글내용</td>
	  		<td>
	  			<textarea name="board_content" id="board_content" rows="10" cols="40"></textarea>
	  		</td>
		</tr>
		...
		<tr>
			<td>암호</td>
			<td>
				<input type="password" name="board_pw" id="board_pw" size="20">
